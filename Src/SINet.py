...
class RRB(nn.Module):
    def __init__(self, in_channel, channel):
        ...
        self.conv2_1 = nn.Conv2d(in_channel, channel,  kernel_size=(1, 3), padding=(0, 1),stride=1, bias=False)
        self.conv2_2 = nn.Conv2d(channel, channel, kernel_size=(3, 1), padding=(1, 0), stride=1, bias=False)
        self.conv3_1 = nn.Conv2d(channel, channel, kernel_size=(1, 3), stride=1, padding=(0, 1), bias=False)
        self.conv3_2 = nn.Conv2d(channel, in_channel, kernel_size=(3, 1), stride=1, padding=(1, 0), bias=False)
        self.conv2 = nn.Conv2d(in_channel, channel, kernel_size=3, padding=1, stride=1, bias=False)
        self.conv3 = nn.Conv2d(channel, in_channel, kernel_size=3, stride=1, padding=1, bias=False)
...
class PDC_SM(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.conv_shortcut0 = nn.Conv2d(channel, 3*channel, 1)
        self.conv_shortcut1 = nn.Conv2d(channel, 4*channel, 1)
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        x3_1 = self.relu(x3+x3_1)
        """x2_1 = self.relu(x2+x2_1)"""
        ...
        """x4 = self.conv_shortcut1(x4)
        x3_2 = self.relu(x4+x3_2)"""
...
class PDC_IM(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.cov_shortcut = nn.Conv2d(channel, 3*channel, 1)
    ...
    def forward(self, x1, x2, x3):
        ...
        """
        x3 = self.cov_shortcut(x3)
        x3_2 = self.relu(x3+x3_2)"""
...
class SINet_ResNet50(nn.Module):
    ...
    def __init__(self, channel=32, opt=None):
        ...
        self.conv_mul = nn.Conv2d(320, 512, 1)
        self.conv_cat = nn.Conv2d(832, 512, 1)
        ...
        #self.rrb1_sm = RRB(320, 160)
        #self.rrb2_sm = RRB(3584, 1792)
        #self.rrb3_sm = RRB(3072, 1536)
        #self.rrb4_sm = RRB(2048, 1024)

        self.rrb1_im = RRB(512, 256)
        self.rrb2_im = RRB(1024, 512)
        self.rrb3_im = RRB(2048, 1024)
    ...
    def forward(self, x):
        ...
        #x01_down = self.rrb1_sm(x01_down)
        ...
        #x2_sm_cat = self.rrb2_sm(x2_sm_cat)
        #x3_sm_cat = self.rrb3_sm(x3_sm_cat)
        #x4_sm = self.rrb4_sm(x4_sm)
